...
void ptp_req_thread_func(timesync_clock_t *instance)
{
    ...
    memcpy(&tx_buf[14], (void*)&req, sizeof(ptp_message_pdelay_req_t));
    while (!instance->stop)
    {
        instance->mutex_lock(instance->mutex);
        int sent = instance->send(tx_buf, sizeof(ptp_message_pdelay_req_t) + 14);
            ...
            switch (header->message_type)
            {
                ...
                case PTP_MESSAGE_TYPE_FOLLOW_UP:
                {
                    ...
                    if (delay_info != NULL)
                    {
                        ...
                        {
                            // TODO: int64 theoretically reduces the possible time range, what do?
